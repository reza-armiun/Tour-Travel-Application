...
public class BookingTourHandler extends QuestionHandler{
    ...
    public BookingTourHandler(BookingService bookingService) {
    ...
    public void run(Map<String, Object> values) {
        ...
        if(!result) {
            ...
            if(selectedIndex == 0) this.next= this.questionHandlers.get(SelectTourTypeHandler.NAME);
            if(selectedIndex == 1) this.next = this.questionHandlers.get(AddTravelerHandler.NAME);
