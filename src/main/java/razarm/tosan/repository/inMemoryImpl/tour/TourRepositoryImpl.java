...
public class TourRepositoryImpl implements TourRepository {
    ...
    private Tour getTour(TourData tourData) {
        ...
        final Set<SchedulePlan> schedulePlans =
                tourData.getSchedulePlans().stream()
                        .map(
                                schedulePlanData -> {
                                    final var accommodation =
                                            accommodationOrderRepository.findById(
                                                    schedulePlanData.getAccommodationOrderId());
                                    final var vehicleOrders =
                                            schedulePlanData.getVehicleOrderIds().stream()
                                                    .map(vehicleOrderRepository::findById)
                                                    .collect(Collectors.toUnmodifiableSet());
                                    final var foodOrders =
                                            schedulePlanData.getFoodOrderIds().stream()
                                                    .map(foodOrderRepository::findById)
                                                    .collect(Collectors.toUnmodifiableSet());
                                    return schedulePlanDataToSchedulePlan
                                            .convert(schedulePlanData)
                                            .cloneWithAccommodation(accommodation)
